/*!
 * @author aphy358 <358937572@qq.com>
 * @url https://github.com/aphy358/Boot.Alert.git
 */
(function ($) {
    /**
    ...
    $.confirm = function (options, e) {
        ...
        modal.on('shown.bs.modal', function () {
            if ($(parent.document.body).find("#BootAlert").length > 0) {
                $(parent.document.body).find("#BootAlert").remove();
            }
        });
    ...
    var o_html = '<div id="BootAlert" hidden="hidden"></div>';
    var o_alert = null;

    BootAlertWarning = function (message, title) {
        var alertParams = {
            text: message,
            title: title,
            style: 'AlertWarning',
    ...
    BootAlertError = function (message, title) {
        var alertParams = {
            text: message,
            title: title,
            style: 'AlertError',
    ...
    BootAlertSuccess = function (message, callback, title) {
        var alertParams;
        if (callback != null) {
            alertParams = {
                text: message,
    ...
    BootConfirm = function (message, callback, title) {
        var alertParams = {
            text: message,
            title: title,
            controlButton: true,
    ...
    function ProcessAlert(alertParams) {
        if ($(parent.document.body).find("#BootAlert").length < 1) {
            $(parent.document.body).append($(o_html));
        }

        o_alert = $(parent.document.body).find("#BootAlert");
